...
# لیست پروتکل‌های قابل شناسایی (به راحتی قابل افزودن پروتکل جدید)
PROTOCOLS = ["slipnet", "stormdns", "cottondns"]

# رجکس دقیق‌تر برای شناسایی همه‌ی پروتکل‌های بالا و نسخه‌ی -enc آن‌ها
PROTOCOL_PATTERN = "|".join(re.escape(p) for p in PROTOCOLS)
SLIPNET_REGEX = re.compile(
    rf'(?:{PROTOCOL_PATTERN})(?:-enc)?:\/\/[^\s<>"\'\[\]{{}}|\\^`]+',
    re.IGNORECASE
)
...
def generate_output(per_channel_data: OrderedDict, total_unique: int) -> str:
    ...
    for ch, links in per_channel_data.items():
        ...
        if count == 0:
            continue
        lines.append(f"☬Node From : {ch} {count} Node")
        lines.append("")
        for link in links:
            lines.append(link)
        lines.append("")
